...
function ProductsSection({ pendingEditId, onConsumePendingEdit } = {}) {
  ...
  const submitForm = () => {
    ...
    // Drop incomplete rows (a label with no value, or vice versa) rather than send them — the
    // backend requires both non-empty.
    const catalog = {
    ...
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit product" : "Add product"} footer={
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            <p className="mb-2 text-sm font-medium text-[var(--text-primary)]">
              Highlights <span className="font-normal text-[var(--text-muted)]">(key selling points, shown as a bullet list)</span>
            </p>
            <div className="flex flex-col gap-2">
